...
**2. Claude Pro at $20 was never addressed.** It was named directly in the original question. The doc compared only against Max and pay-as-you-go, which set up a strawman ("cancel everything") rather than the realistic alternative ("step down a tier"). Raised by product-reviewer. Fixed by adding a section that prices Pro against the usage profile and concludes it does not fit, plus a sensitivity table showing where the break-even sits.
...
**Where the break-even sits.** Staff-engineer computed roughly 5% of current volume using the blended all-Sonnet figure. The doc's sensitivity table says roughly 3% using the profile's actual model mix. Both are right for their assumptions. The actual-mix basis was kept because the practical question is about a real usage pattern, not a hypothetical all-Sonnet workload. The distinction does not affect the decision, since both are far below current volume.
...
2. Add a Claude Pro section with a verdict. Directly answers a question that was asked. **Done.**
...
5. Add subscription price comparison for OpenAI and Google. The question covered other providers and only got an auth-compatibility table. **Done.**
...
| Field | Value |
|---|---|
| **What this is** | Run metadata for the smart-review process. Kept so cost and quality can be reviewed later. |
| Type | smart-review |
| Date | 2026-08-06T08:59-0700 |
| Reviewed | `research/opencode-vs-subscriptions.md` |
| Agents | 3, all on Sonnet: staff-engineer, domain-expert, product-reviewer |
